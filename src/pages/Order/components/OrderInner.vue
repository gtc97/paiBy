    ...
    <div class="order-inner">
        <router-link to="/address" class="order-address">
            ...
            <div class="address-content">
                ...
                <p
                    class="address"
                >{{ address.provinceName + address.cityName + address.userAddress }}</p>
            </div>
            <van-icon class="address-right-icon" name="arrow" color="#999" size="0.3rem" />
        </router-link>
            ...
            <van-radio-group v-model="radio" class="pay-type-list">
                <div v-for="item of payTypeList" :key="item.type" class="pay-type-item">
                    <div v-if="item.type == 2">
                        ...
                        <van-radio :name="item.type" class="icon-right" checked-color="#FDB428" />
                    </div>
        ...
        <van-dialog v-model="show" title="上传付款截图" show-cancel-button @confirm="handleSave">
            <van-uploader
                v-model="fileList"
                :max-count="1"
                style="margin: auto;"
...
export default {
            ...
            for (let k = 0; k < this.userPayInfo.length; k++) {
                ...
                for (let kl = 0; kl < this.payType.length; kl++) {
                    if (this.userPayInfo[k].type == this.payType[kl].type) {
                        this.userPayInfo[k].iconClass = this.payType[
                            kl
                        ].iconClass;
    ...
    methods: {
        ...
        handleSave() {
            ...
            this.$api.payPic({ id: this.goodDetails[0].id, payPic: this.payImgUrl }).then(res => {
                    if (res.status === 1) {
                        Toast(res.msg);
                        this.$router.push({ path: "/my" });
                    } else {
...
.order-inner {
    ...
    .order-address {
        ...
        padding: 0.52rem /* 39/75 */ 0.8667rem /* 65/75 */ 0 1.2933rem
            /* 97/75 */;
        margin: auto;
        width: 9.2rem /* 690/75 */;
        height: 2rem /* 150/75 */;
